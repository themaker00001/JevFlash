...
Device-agnostic fork: runs on CUDA, Apple MPS, or CPU (the upstream reference
only supports CUDA). Also supports datasets with no teacher distribution by
training on one-hot gold labels (--objective gold).
"""
import argparse
import hashlib
...
def autocast_ctx(device):
    ...
    if device.type == 'mps':
        # bf16 kernels are incomplete on MPS as of torch 2.14; fp32 keeps numerics stable.
...
def load_examples(path, tokenizer, max_length):
    ...
    for line in Path(path).read_text().splitlines():
        ...
        for qid, q in row['questions'].items():
            ...
            raw, target, rounding = None, None, None
            if teacher is not None:
                observed = teacher['native_probs'][qid]
                ...
                decimals = teacher.get('rounding', {}).get('probabilityDecimals')
            ...
            ex = dict(id=f"{row['id']}:{qid}", state_id=row['state_id'], family_id=row['family_id'],
                      ...
                      teacher_rounding=rounding, source=row, candidate_texts=texts)
            examples.append(ex)
            audit.append(dict(id=ex['id'], split=ex['split'], type=typ, k=len(ids),
                              teacher_raw_sum=sum(raw) if raw else None,
                              teacher_proxy_usable=target is not None,
                              teacher_argmax_matches_gold=(max(range(len(ids)), key=raw.__getitem__) == gold) if raw else None,
                              target_transform=('identity_rounded_proxy' if target is not None
                                                 else ('quarantined' if teacher is not None else 'no_teacher'))))
...
@torch.no_grad()
def evaluate(model, examples, pad_token, batch, device, path=None):
    model.eval()
    records = []
...
@torch.no_grad()
def frozen_native_baseline(lm, tokenizer, examples, batch, path, use_chat, device):
    ...
    for start in range(0, len(examples), batch):
        ...
        lengths = torch.tensor(list(map(len, paths)), device=device)
        tokens = torch.full((len(paths), int(lengths.max())), tokenizer.pad_token_id, device=device, dtype=torch.long)
        for i, ids in enumerate(paths):
            tokens[i, :len(ids)] = torch.tensor(ids, device=device)
        mask = torch.arange(tokens.shape[1], device=device)[None, :] < lengths[:, None]
        with autocast_ctx(device):
            h = lm.model(input_ids=tokens, attention_mask=mask, use_cache=False).last_hidden_state
            h = h[torch.arange(len(paths), device=device), lengths-1]
...
@torch.no_grad()
def benchmark(model, examples, pad_token, device):
    model.eval()
    states = {}
    for ex in examples:
    ...
    return {'scope': 'warm inference including tensor assembly and device copies; excludes tokenization/network',
            'device': device.type, 'prefix_sharing': False, 'autoregressive_decode_steps': 0, 'results': results}
...
def main():
    ...
    p.add_argument('--objective', choices=['teacher', 'gold'], default='gold')
    ...
    if args.objective == 'teacher' and not any(e['teacher_probs'] is not None for e in bysplit['dev']):
        raise ValueError('No eligible dev teacher targets: use --objective gold for datasets without teacher labels')
    ...
    final = evaluate(model, evaluation, tokenizer.pad_token_id, args.batch_questions, device, out/'predictions.jsonl')
    timing = benchmark(model, bysplit['test'], tokenizer.pad_token_id, device)
    ...
    dump(out/'summary.json', {'best_step': best_step, 'selected_on': 'dev teacher CE' if args.objective == 'teacher' else 'dev gold NLL',
                            ...
                            'max_gpu_allocated_gb': (torch.cuda.max_memory_allocated()/1e9) if device.type == 'cuda' else None})
